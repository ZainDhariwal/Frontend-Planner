...
export default function NodeEditor({ 
  ...
  const generateAgentPrompt = () => {
    ...
    prompt += `Target Framework: ${fName}\n`;
    
    if (framework === "vue") {
      prompt += `Preferred Format: Vue Single File Component (.vue) using Composition API with <script setup lang="ts"> and Tailwind CSS.\n`;
    } else if (framework === "svelte") {
      prompt += `Preferred Format: Svelte Component (.svelte) using Svelte 5 Runes ($props, $state, etc.) with <script lang="ts"> and Tailwind CSS.\n`;
    } else {
      prompt += `Preferred Format: React Component (.tsx) with TypeScript and Tailwind CSS.\n`;
    }
    ...
    if (node.type === "page") {
      ...
      if (framework === "vue") {
        prompt += `Expected Inputs (composable parameters): ${metadata.inputs || "None"}\n`;
        prompt += `Expected Outputs (composable returns): ${metadata.outputs || "None"}\n`;
      } else {
      ...
      if (framework === "vue") {
        prompt += `State Shape (Pinia store structure): ${metadata.valueShape || "None"}\n`;
      } else if (framework === "svelte") {
        prompt += `State Shape (Svelte writable/context stores): ${metadata.valueShape || "None"}\n`;
      } else {
    ...
    if (framework === "vue") {
      prompt += `\nTask: Please write the complete, clean Vue SFC code for this ${node.type === "context" ? "Pinia store / context" : node.type === "hook" ? "composable" : node.type}. Do not use mock placeholders. Ensure proper typing, clean styles, and correct lifecycle imports.`;
    } else if (framework === "svelte") {
      prompt += `\nTask: Please write the complete, clean Svelte code for this ${node.type === "context" ? "Svelte store / context" : node.type}. Do not use mock placeholders. Ensure proper runes usage and clean markup.`;
    } else {
      prompt += `\nTask: Please write the complete, clean React/TypeScript code for this ${node.type}. Do not use mock placeholders. Ensure proper hooks usage and clean tailwind markup.`;
    }
    return prompt;
  };
        ...
        {readOnly ? (
          <div className="space-y-3">
            <div className="space-y-1">
              ...
              <div className="p-2.5 border border-border bg-muted/20 rounded-xl text-sm font-semibold text-foreground">{name}</div>
            </div>
            <div className="space-y-1">
              <label className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Description</label>
              <div className="p-2.5 border border-border bg-muted/20 rounded-xl text-xs leading-relaxed text-muted-foreground whitespace-pre-wrap">{description || "No description provided."}</div>
            </div>
          </div>
        ) : (
          <div className="space-y-3">
            ...
            <>
              {node.type === "page" && (
                <div className="space-y-1">
                  ...
                  <div className="p-2.5 border border-border bg-muted/20 rounded-xl text-xs font-mono text-foreground">{metadata.path || "/"}</div>
                </div>
              )}
              ...
              {node.type === "component" && (
                <div className="space-y-4">
                  <div className="space-y-1">
                    ...
                    <div className="p-2.5 border border-border bg-muted/20 rounded-xl text-xs capitalize text-foreground">{metadata.atomicType || "atom"}</div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                      {(!metadata.props || metadata.props.length === 0) ? (
                        <div className="text-xs text-muted-foreground p-3 border border-border rounded-xl bg-muted/10 text-center">No properties defined</div>
                      ) : (
                        metadata.props.map((prop: any, index: number) => (
                          <div key={index} className="border border-border bg-muted/20 p-2.5 rounded-xl text-xs flex justify-between gap-2 items-center">
                            <span className="font-semibold text-foreground">{prop.name}</span>
                            <span className="font-mono text-[10px] text-purple-500 bg-purple-500/10 px-1.5 py-0.5 rounded border border-purple-500/20">{prop.type}</span>
                            <span className="text-muted-foreground text-[10px] truncate max-w-[100px]" title={prop.description}>{prop.description}</span>
                          </div>
                        ))
                      )}
                    </div>
              ...
              {node.type === "hook" && (
                <div className="space-y-3">
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-muted-foreground uppercase tracking-widest">{framework === "vue" ? "Composable Parameters" : "Input Parameters"}</label>
                    <div className="p-2.5 border border-border bg-muted/20 rounded-xl text-xs font-mono text-foreground">{metadata.inputs || "None"}</div>
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-muted-foreground uppercase tracking-widest">{framework === "vue" ? "Composable Returns" : "Output / Return Value"}</label>
                    <div className="p-2.5 border border-border bg-muted/20 rounded-xl text-xs font-mono text-foreground">{metadata.outputs || "None"}</div>
                  </div>
                </div>
              )}
              ...
              {node.type === "context" && (
                <div className="space-y-1">
                  <label className="text-xs font-bold text-muted-foreground uppercase tracking-widest">
                    {framework === "vue" ? "Pinia Store Value Shape" : framework === "svelte" ? "Svelte Store Value Shape" : "Value Shape Interface"}
                  </label>
                  <pre className="p-3 border border-border bg-muted/30 rounded-xl text-xs font-mono overflow-x-auto whitespace-pre leading-relaxed select-all">
                    {metadata.valueShape || "None"}
                  </pre>
                </div>
              )}
              ...
              {node.type === "mock_data" && (
                <div className="space-y-1">
                  ...
                  <pre className="p-3 border border-border bg-muted/30 rounded-xl text-xs font-mono overflow-x-auto whitespace-pre leading-relaxed select-all">
                    {typeof metadata.jsonData === "string" ? metadata.jsonData : JSON.stringify(metadata.jsonData || [], null, 2)}
                  </pre>
                </div>
              )}
            </>
