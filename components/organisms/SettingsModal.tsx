...
export default function SettingsModal({ isOpen, onClose, onSettingsChanged }: SettingsModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      {/* Modal Card */}
      <div className="relative border border-border bg-card/90 backdrop-blur-xl rounded-2xl w-full max-w-md p-6 shadow-2xl z-10 animate-in fade-in zoom-in-95 duration-200">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-border pb-4 mb-5">
          <div className="flex items-center gap-2">
            <Key className="h-5 w-5 text-blue-500 dark:text-blue-400" />
            <h3 className="font-bold text-lg text-foreground">Planner Settings</h3>
          </div>
          <button 
            onClick={onClose} 
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Default LLM Provider</label>
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between items-center">
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Gemini API Key (Paid/Custom)</label>
              <a 
            ...
            <input
              ...
              className="w-full border border-border bg-background rounded-xl px-3.5 py-2 text-sm text-foreground outline-none focus:border-zinc-400 dark:focus:border-zinc-700 transition-all placeholder:text-muted-foreground/40"
            />
          </div>
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between items-center">
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Anthropic API Key (Custom)</label>
              <a 
          ...
          {/* Security Notice */}
          <div className="border border-border bg-muted/40 p-3.5 rounded-xl flex items-start gap-2.5">
